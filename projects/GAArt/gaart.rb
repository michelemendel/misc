...
class ImageWindow < FXMainWindow
    ...
    def rMagickImg2FxColors(pic)
        fxColors = []
        pic.getPixels.each do |pxl|
            fxColors << FXRGB(pxl.red, pxl.green, pxl.blue)
        end
        return fxColors
    end
    
    def drawImage(dc, pic)
        img = FXImage.new(getApp(), rMagickImg2FxColors(pic), IMAGE_OWNED|IMAGE_DITHER|IMAGE_SHMI|IMAGE_SHMP, @imgWidth, @imgHeight)
        img.create
